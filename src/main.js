...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let tasks = StorageAdapter.getTasks();
    ...
    const renderTasks = () => {
        ...
        tasks.forEach((task, index) => {
            ...
            taskItem.className = `flex justify-between items-center border rounded p-2 ${task.completed ? 'bg-gray-100 text-gray-500' : 'bg-white'
                }`;
            const info = document.createElement('div');
            info.className = `flex flex-col ${task.completed ? 'line-through' : ''}`;
            info.innerHTML = `
